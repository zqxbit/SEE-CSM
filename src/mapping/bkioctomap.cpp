...
#pragma omp critical
    ...
    void SemanticBKIOctoMap::insert_map_point_multi(pcl::PointCloud<MapPoint>::Ptr transformed_cells)
    {
        ...
        std::vector<semantic_bki::point3f>  common_node; // 两个地图共有的block，用于之后的连续化建图
...
#pragma omp critical
            ...
            if ( std::count(new_block.begin(), new_block.end(), key) == 0 )
             {
                ...
                if ( if_free_node == 0 ) // 这个节点也是非空的时候才比较
                {
                    ...
                    old_prob.push_back(probs[semantic]);
                    semantic_bki::point3f node_center;
                    node_center.x() = it->x;
                    node_center.y() = it->y;
                    node_center.z() = it->z;
        ...
        {
            semantic_bki::point3f p = common_node[i];
    ...
    int SemanticBKIOctoMap::block_is_unknow(const BlockHashKey &key) {
        ...
        {
            if( it.get_node().get_state() != semantic_bki::State::UNKNOWN )
                return 0;
        }
        return 1;
    }
...
//----------------------------------------------------------------------------------------------------------------------------------------------------//
            ...
            if (block_xy_hits.size() >= 1)   // 妈的这里导致free不行
            {
